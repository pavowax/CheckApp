...
from pkg.ArjunMaster.arjun.core.colors import green, end, info, bad, good, run, res
...
from pkg.ArjunMaster.arjun.core.exporter import exporter
from pkg.ArjunMaster.arjun.core.anomaly import define, compare
from pkg.ArjunMaster.arjun.core.utils import fetch_params, stable_request, random_str, slicer, confirm, populate, reader, nullify, prepare_requests, compatible_path

from pkg.ArjunMaster.arjun.plugins.heuristic import heuristic
...
from pkg.ArjunMaster.arjun.core.bruter import bruter
...
def initialize(request, passive=False, single_url=False):
    """
    handles parameter finding process for a single request object
    returns 'skipped' (on error), list on success
    """
    wordlist_file = '/app/pkg/ArjunMaster/arjun/db/small.txt'
    wordlist_file = compatible_path(wordlist_file)
    wordlist = set(reader(wordlist_file, mode='lines'))
    if passive:
        host = request['url']
    ...
    if not request['url']:
        ...
        with open(f'/app/pkg/ArjunMaster/arjun/db/special.json', 'r') as f:
            populated.update(json.load(f))
        param_groups = slicer(populated, int(len(wordlist)/10))
