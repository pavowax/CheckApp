...
globals()["_headers"] = dict(filter(lambda _: _[1], ((COOKIE, options.cookie), (UA, options.ua or NAME), (REFERER,  options.referer))))
urllib.request.install_opener(urllib.request.build_opener(urllib.request.ProxyHandler({'http': options.proxy})) if options.proxy else None)
            ...
            for match in re.finditer(r"((\A|[?&])(?P<parameter>[^_]\w*)=)(?P<value>[^&#]+)", current):
                ...
                for (dbms, regex) in ((dbms, regex) for dbms in DBMS_ERRORS for regex in DBMS_ERRORS[dbms]):
                    if not vulnerable and re.search(regex, content[HTML], re.I) and not re.search(regex, original[HTML], re.I):
                        # result=f"{phase} parameter '{match.group("parameter")}' appears to be error SQLi vulnerable ({dbms})"
                        # sqli[match.group("parameter")]=result
                        sqli["parameter"]=f"parameter of {match.group("parameter")}  appears to be SQLi vulnerable ({dbms})"
                        sqli["dbms"]=f"{dbms}"
...
# print(scan_page("http://192.168.1.3:1337/lab/sql-injection/find-password/?search="))
